...
GOLDFLAGS_BASE  := -X github.com/Orca18/go-novarand/config.BuildNumber=$(BUILDNUMBER) \
		 -X github.com/Orca18/go-novarand/config.CommitHash=$(COMMITHASH) \
		 -X github.com/Orca18/go-novarand/config.Branch=$(BUILDBRANCH) \
		 -X github.com/Orca18/go-novarand/config.DefaultDeadlock=$(DEFAULT_DEADLOCK) \
		 -extldflags \"$(EXTLDFLAGS)\"

GOLDFLAGS := $(GOLDFLAGS_BASE) \
		 -X github.com/Orca18/go-novarand/config.Channel=$(CHANNEL)
...
%/msgp_gen.go: deps ALWAYS
		@set +e; \
		printf "msgp: $(@D)..."; \
		$(GOPATH1)/bin/msgp -file ./$(@D) -o $@ -warnmask github.com/Orca18/go-novarand > ./$@.out 2>&1; \
		if [ "$$?" != "0" ]; then \
			printf "failed:\n$(GOPATH1)/bin/msgp -file ./$(@D) -o $@ -warnmask github.com/Orca18/go-novarand\n"; \
			cat ./$@.out; \
			rm ./$@.out; \
			exit 1; \
		else \
