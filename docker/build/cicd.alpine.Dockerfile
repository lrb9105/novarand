...
COPY . $GOPATH/src/github.com/Orca18/go-novarand
WORKDIR $GOPATH/src/github.com/Orca18/go-novarand
ENV GCC_CONFIG="--with-arch=armv6" \
    GOPROXY=https://proxy.golang.org,https://pkg.go.dev,https://goproxy.io,direct
RUN make clean
RUN rm -rf $GOPATH/src/github.com/Orca18/go-novarand && \
    mkdir -p $GOPATH/src/github.com/Orca18/go-novarand
CMD ["/bin/bash"]
